...
We report for each instance the objective value, solve time, and MIP gap (when available) for all three methods. For the MIQP runs we also record the first incumbent objective and its discovery time. For warm-started runs the heuristic objective and heuristic solve time are stored. After the pull of 31~Aug~2026 the vertex-based sweep is complete (540 configs $\times$ 3 methods = 1620 runs, 1703 rows including heuristic, Gurobi 11.0, TL=1800\,s, MIPGap=0); the edge-based sweep is partial (334 configs, 1001 runs, $n_r=1,2,3$ fully and $n_r=5$ partially with $h=1,2$, see Table~\ref{tab:results-vertex-edge}). All figures and tables below are generated from \texttt{compare\_results/results.csv} and \texttt{compare\_results\_edges/results.csv}.
...
\begin{figure}[t]
  ...
  \caption{Mean solve time (log scale) vs.\ number of regions $n_r$, averaged over $n_h$, $E$ and seeds. Vertex-based (Vertex-Approach, MTZ) vs.\ edge-based (Edges-Approach, DFJ lazy) with and without warm-start, plus heuristic. Edge results are partial: $n_r=8,10$ not yet executed.}
...
\begin{figure}[t]
  ...
  \caption{Objective parity check: Vertex-Approach (MTZ) vs.\ Edges-Approach (DFJ) on the 325 (cold) / 357 (warm) configs where both formulations finished. Diagonal = equivalence. Mean ratio Edges-Approach/Vertex-Approach = 1.021 (cold) and 1.002 (warm), confirming the two MIQPs are equivalent up to solver variability and time-limit effects; warm-start closes the residual gap.}
...
The aggregated results (Tables~\ref{tab:results-summary-rings}--\ref{tab:results-vertex-edge} and Figs.~\ref{fig:compare-time}--\ref{fig:compare-incumbent}) confirm three patterns. First, the heuristic is consistently feasible in $<0.6$\,s (mean 0.03\,s for $n_r=1$ up to 0.54\,s for $n_r=10$) and its objective is within 2--8\% of the exact solver on small instances where optimality is proven. Second, warm-starting (R+WS, E+WS) reduces the mean time to the first feasible incumbent by one to two orders of magnitude, especially for tight endurances (Fig.~\ref{fig:compare-incumbent}); for $n_r=10$ the mean MIP gap drops from 61.3\% (R) to 42.2\% (R+WS) at the 1800\,s limit. Third, on the 325/357 configs where both formulations were executed, Vertex-Approach and Edges-Approach are equivalent in objective (mean ratio 1.02 cold, 1.00 warm, Fig.~\ref{fig:compare-parity}); the edge-based model uses $\approx$2--5\% fewer variables but incurs DFJ lazy-cut overhead, so its mean solve time is comparable or slightly higher on $n_r\le3$ (Fig.~\ref{fig:compare-time}). For $n_r=5$ the edge-based sweep is still partial (191/270 configs, only $h=1,2$), and for $n_r=8,10$ no edge results are available yet; completing the sweep will allow a definitive scalability comparison.
...
On the completed vertex-based sweep (540 configs) the heuristic is always feasible and warm-starting consistently improves both time-to-first-incumbent and final gap; the edge-based formulation shows equivalent solution quality on the subset executed so far (mean warm ratio 1.002) and will be fully benchmarked once the $n_r=5,8,10$ sweeps complete.
