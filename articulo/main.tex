...
\emph{Idea.} Once vertices have positions, the drone must string them into depot tours: each operation $o$ is a closed walk starting and ending at $v_0$ that visits a subset of vertices, every vertex is visited exactly once across all operations, and no tour contains a cycle detached from the depot. We model this with traversal variables $x_{vv'}^o$ (which legs are flown in which operation) and visitation variables $y_v^o$ derived from them; flow conservation (Path-C$_2$) plus depot departure/arrival (Path-C$_1$, Path-C$_3$) shape each operation, while Miller--Tucker--Zemlin ordering variables $u_v^o$ (Path-C$_7$) rule out subtours. Operations are allowed to be idle once everything is covered, via the flag $\zeta^o$.
...
\emph{Idea.} The path constraints above admit many symmetric solutions (e.g.\ permuting idle operations) that slow down branch-and-bound without changing the optimum. The inequalities VI-C$_1$--C$_4$ cut off such fractional and symmetric solutions: they force idle operations to come last, count how many vertices each operation covers, and tie that count to the idle flag so the relaxation ``sees'' that an active operation must do useful work.
...
\emph{Idea.} Routing needs leg lengths, but vertex positions are variables, so distances cannot be precomputed---except along a chain, where the $\lambda$ ordering makes them trivial. The trick is therefore twofold: on intra-region edges read the length off an accumulated-distance variable $dfs_v$ (a difference of two such variables, Dist-C$_1$--C$_2$), and on inter-region legs introduce explicit distance variables bounded below by the Euclidean norm via a second-order cone (Dist-C$_3$--C$_6$). Since the objective minimizes total length, each ``$\ge$'' bound is tight at optimality.
...
The energy consumed by the drone is modelled using the drag formula $w = \tfrac12 A c\,\rho\,\nu\,d$, where $A$ is the frontal area, $c$ the drag coefficient, $\rho$ the air density, $\nu$ the drone speed, and $d$ the travelled distance. We define the constants $E^{xy}=E^{z}=\tfrac12 cA$ for brevity. \emph{Units:} throughout Sections~\ref{sec:formulation}--\ref{sec:splitpoints} energy is measured in the model's nominal units (drag constants at cruise speed, calm air), which we write as pseudo-joules to distinguish them from the real joules of the wind-aware extension in Section~\ref{sec:windaware}. As with distances, we treat intra-region and inter-region pairs separately.
...
\emph{Idea.} Flying from region $r$ to region $r'$ burns horizontal energy at the air density of the departure altitude and vertical energy only for the climbed part. Both depend on decisions: the departure chain (via $\alpha_t$) selects the density, and the relative heights of $r$ and $r'$ decide the climb direction. Two auxiliary binaries disentangle this: $\varphi_{rr'}$ is one if the drone departs from region $r$ at the altitude selected for that region, and $\psi_{rr'}$ is one if the altitude of $r$ is lower than that of $r'$. Each branch of~\eqref{eq:ener_ext} below is then a product of binaries with distances, linearized exactly with Fortet envelopes. The energy consumption is then:
...
\emph{Idea.} An operation is feasible only if the energy of the legs it actually flies fits the battery. That is a product of two variables (fly the leg $\times$ leg cost), so we introduce one auxiliary $\ell_{uv}^o$ per leg--operation pair that equals $w_{uv}$ when $x_{uv}^o=1$ and $0$ otherwise (big-M, End-C$_1$--C$_4$); the budget $W^{\max}$ then caps the sum per operation (End-C$_5$). The same device serves the objective, which needs $x_{uv}^o\cdot d_{uv}$.
...
\emph{Idea.} A ring covered as one loop forces its entry and exit points to coincide, so a tour can never slip out mid-ring to visit a neighbour and come back. Allowing $K$ arcs per ring (Fig.~\ref{fig:model-k}) turns each ring into a sequence of pieces joined by zero-cost connectors; pieces of the same ring may then live in different operations or be interleaved with other regions inside one operation. Section~\ref{sec:k2-example} walks through a concrete $K=2$ solution arc by arc.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=0.82, every node/.style={font=\footnotesize}]
    % --- ring (coverage loop) ---
    \draw[gray!50, thin] (0,0) circle (2.2);
    % arc 1: S (180deg) clockwise through top to C (55deg), 125deg ~ 35%
    \draw[blue, line width=2.2pt, ->, >=stealth] (180:2.2) arc (180:55:2.2);
    % arc 2: C clockwise through right+bottom back to S, 235deg ~ 65%
    \draw[red, line width=2.2pt, ->, >=stealth] (55:2.2) arc (55:-180:2.2);
    % vertices
    \fill (180:2.2) circle (2.4pt);
    \node[left=2pt] at (180:2.2) {$\ell_1\,(\lambda{=}0)$};
    \node[below left=1pt] at (180:2.2) {$r_2\,(\lambda{=}4)$};
    \fill (55:2.2) circle (2.4pt);
    \node[above right=1pt] at (55:2.2) {$r_1{=}\ell_2\,(\lambda{=}1.39)$};
    \node[blue] at (115:2.75) {arc 1};
    \node[red] at (-25:2.75) {arc 2};
    % zero-length connectors (dashed loops)
    \draw[dashed, gray, ->, >=stealth] (55:2.2) ++(0.12,0.28) arc (90:-180:0.28);
    \node[gray, font=\scriptsize] at (1.95,2.15) {conn.\ $0$};
    \draw[dashed, gray, ->, >=stealth] (180:2.2) ++(-0.30,-0.10) arc (180:-90:0.28);
    \node[gray, font=\scriptsize] at (-3.05,-0.95) {wrap $0$};
    % satellite region + interleaving transit
    \draw[fill=green!10] (4.0,0.9) rectangle (4.9,1.8);
    \node[font=\scriptsize] at (4.45,1.35) {satellite};
    \draw[green!60!black, dashed, ->, >=stealth] (55:2.2) -- (4.05,1.15)
      node[midway, above, font=\scriptsize] {exit};
    \draw[green!60!black, dashed, ->, >=stealth] (4.05,1.6) -- (55:2.2)
      node[midway, below, font=\scriptsize] {re-enter};
    % depot + first entry / final return
    \draw[fill=black!10] (-4.9,-2.1) rectangle (-4.1,-1.3);
    \node[font=\scriptsize] at (-4.5,-1.7) {depot};
    \draw[black, ->, >=stealth] (-4.05,-1.55) -- (180:2.2)
      node[midway, below, font=\scriptsize] {enter};
    \draw[black, ->, >=stealth] (180:2.2) ++(-0.05,-0.35) -- (-4.05,-1.95)
      node[midway, below, font=\scriptsize] {return};
    % lambda scale
    \draw[->] (-2.6,-3.3) -- (2.6,-3.3) node[right, font=\scriptsize] {$\lambda$};
    \foreach \l/\x in {0/-2.6, 1.39/-0.79, 4/2.6} {
      \draw (\x,-3.3) -- (\x,-3.15);
      \node[below, font=\scriptsize] at (\x,-3.32) {$\l$};
    }
  \end{tikzpicture}
  \caption{A ring split into $K=2$ arcs (numbers from the demo solution of Fig.~\ref{fig:demo-k2}). The tour enters at $\ell_1$ ($\lambda=0$, S-C$_1$), covers arc~1, exits mid-ring at $r_1=\ell_2$ ($\lambda=1.39$, S-C$_2$) to visit the satellite, re-enters at the same point via the zero-length connector, covers arc~2, and returns to the depot from $r_2$ ($\lambda=4$, S-C$_3$); S-C$_4$ keeps every arc non-empty in the $\lambda$ order.}
  \label{fig:model-k}
\end{figure}
...
Table~\ref{tab:k2-walk} walks through the single operation of Fig.~\ref{fig:demo-k2} arc by arc (vertex indices decoded from the solver output; $\lambda$ cuts verified against the solution file). The tour covers the satellite rings opportunistically between pieces of the central region R0: R0-ring1 is split across steps~2 and~4, R0-ring0 across steps~6 and~8. Each re-entry reuses the cut point as both exit and entry ($r_1=\ell_2$ in Fig.~\ref{fig:model-k}), so no distance is wasted. With $K=1$ this tour is infeasible: each ring would have to be covered in one contiguous visit with coincident entry/exit, so R0 could not be interleaved with the satellites (the $K=1$ optimum on this instance is 677.6\,m---25.5\,m more, Table~\ref{tab:k-sweep}).
...
\begin{table}[t]
  ...
  \begin{tabular}{lp{7.2cm}r}
    ...
    P3 coord-descent & 7 entry points moved toward neighbouring visits & 311.3 \\
