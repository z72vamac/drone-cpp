...
\input{tables/table_results_summary}
\input{tables/table_results_vertex_edge}
\input{tables/table_results_by_endurance}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{pictures/compare_time_by_regions.pdf}
  \caption{Mean solve time (log scale) vs.\ number of regions $n_r$, averaged over $n_h$, $E$ and seeds. Vertex-based (Vertex-Approach, MTZ) vs.\ edge-based (Edges-Approach, DFJ lazy) with and without warm-start, plus heuristic, on the full testbed.}
...
\begin{figure}[t]
  ...
  \caption{Mean MIP gap at the 1800\,s limit. Both formulations prove optimality for $n_r=1$; the gap grows with $n_r$. Warm-start reduces the gap for large instances (e.g.\ $n_r=10$: Vertex-Approach 61.3\% $\to$ Vertex-Approach+WS 42.2\%).}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{pictures/compare_perfprofile.pdf}
  \caption{Performance profiles (Dolan--Mor\'e) of solution quality: fraction of configs where each formulation is within a factor $\tau$ of the best objective found by either. Unsolved runs count as infinite ratio, so each curve plateaus at the fraction of configs solved. Cold (left): the vertex-based formulation dominates (100\% solved, best on 98\% of configs at $\tau=1$ vs.\ 41\%). Warm-started (right): near-tie, with 97--99\% of configs within 5\% of the best for both.}
  \label{fig:compare-perfprofile}
\end{figure}

\begin{figure}[t]
  \centering
  \includegraphics[width=0.85\linewidth]{pictures/compare_gapcdf.pdf}
  \caption{Distribution of final MIP gaps over the 540 configs (runs with no solution counted at 100\%). Warm-starting shifts both formulations markedly left: the fraction under 50\% gap rises from 76\% to 93\% (vertex) and from 57\% to 90\% (edges).}
...
The aggregated results (Tables~\ref{tab:results-summary-rings}--\ref{tab:results-vertex-edge} and Figs.~\ref{fig:compare-time}--\ref{fig:compare-incumbent}) confirm three patterns. First, the heuristic is consistently feasible in $<0.6$\,s (mean 0.03\,s for $n_r=1$ up to 0.54\,s for $n_r=10$) and its objective is within 2--8\% of the exact solver on small instances where optimality is proven. Second, warm-starting (R+WS, E+WS) reduces the mean time to the first feasible incumbent by one to two orders of magnitude, especially for tight endurances (Fig.~\ref{fig:compare-incumbent}); the effect is strongest for the edge-based formulation, whose cold runs need 45\,s ($n_r=2$) to 1101\,s ($n_r=10$) for the first incumbent against 0.00\,s and 7.7\,s warm-started. For $n_r=10$ the mean MIP gap drops from 61.3\% (R) to 42.2\% (R+WS) at the 1800\,s limit. Third, the head-to-head comparison of the two formulations is best read from the performance profiles of Fig.~\ref{fig:compare-perfprofile}, which are robust to the censoring that biases plain means (cold Edges-Approach leaves most large instances unsolved). Without warm-start (509 configs where at least one formulation solved), the vertex-based formulation dominates: it solves 100\% of configs and is best-or-tied on 98\% already at $\tau=1$ (100\% by $\tau=1.05$), while the edge-based one is best on only 41\% and plateaus at 61\% solved, with worst-case ratios up to 1.83. With warm-start (530 configs) the picture flips to a near-tie: the vertex approach still wins outright more often (93\% vs.\ 57\% at $\tau=1$), but by $\tau=1.05$ both lie within 5\% of the best on 97--99\% of configs, and both solve essentially everything (99.6\% vs.\ 100\%). The parity scatter (Fig.~\ref{fig:compare-parity}, mean ratio 1.023 cold on 311 configs, 1.003 warm on 528) and the warm-started agreement within 1\% on the largest instances ($n_r=8$: 1649.5 vs.\ 1647.5; $n_r=10$: 2032.6 vs.\ 2017.2) confirm the same equivalence. The edge-based model uses $\approx$2--5\% fewer variables but incurs DFJ lazy-cut overhead: it is markedly slower without warm-start on small instances (172.6\,s vs.\ 85.1\,s at $n_r=1$; 872.5\,s vs.\ 454.3\,s at $n_r=2$), while on large instances both hit the time limit. The gap distributions of Fig.~\ref{fig:compare-gapcdf} show the same story from the dual side: warm-starting moves the mass left for both formulations, most dramatically for edges (fraction under 50\% gap from 57\% to 90\%).
...
On the completed vertex-based sweep (540 configs) the heuristic is always feasible and warm-starting consistently improves both time-to-first-incumbent and final gap; the edge-based formulation, now also benchmarked on the full 540-config testbed, is equivalent in solution quality once warm-started (performance profiles within 5\% of best on 97--99\% of configs; mean warm ratio 1.003) but strictly dominated without warm-start, where it solves only 61\% of configs against 100\% for the vertex-based model.
...
\begin{itemize}
    ...
    \item Performing a polyhedral comparison of the MTZ and DFJ lazy relaxations to explain the primal strength of the edge-based formulation on large instances.
\end{itemize}
