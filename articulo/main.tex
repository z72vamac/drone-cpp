...
The constraint D$_{\text{path}}$-C$_8$ (each intra edge traversed exactly once) is omitted: it follows from EC$_1$ together with EC$_{2a}$ and EC$_3$, which enforce $y_{uv}^o = x_{uv}^o$ and $\sum_o y_{uv}^o = 1$, implying $\sum_o x_{uv}^o = 1$.
...
Constraints~\eqref{eq:ec1}--\eqref{eq:ec3} enforce $\sum_o y_{uv}^o = 1$ and $y_{uv}^o = x_{uv}^o$, which gives $y_{uv}^o = x_{uv}^o$ and therefore $\sum_o x_{uv}^o = 1$ (the omitted D$_{\text{path}}$-C$_8$). Moreover, each intra-ring edge in the vertex-based formulation contributes exactly one to $k^o$ via the two vertices it connects (each vertex has degree~1 in an active operation), whereas here it contributes directly via $y_{uv}^o$. The factor of~2 difference between $|\mathcal V'|$ and $|\mathcal E^{int}|$ in VI-1 is absorbed by the halved $k^o$ values. Consequently, the two formulations share the same set of feasible integer solutions and therefore the same optimal value. The edge-based formulation is implemented alongside the vertex-based one in our computational framework, allowing a direct comparison of their performance.
