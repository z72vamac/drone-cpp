...
\begin{frame}{Endurance \& objective}
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.52\textwidth}
      ...
      {\footnotesize
      \begin{align*}
        \ell_{uv}^o &\le x_{uv}^oM \tag{End-C$_1$}\\
        \ell_{uv}^o &\le w_{uv} \tag{End-C$_2$}\\
        \ell_{uv}^o &\ge w_{uv}-(1-x_{uv}^o)M \tag{End-C$_3$}\\
        \ell_{uv}^o &\ge 0 \tag{End-C$_4$}\\
...
\begin{frame}{Results — aggregated means (vertex, complete)}
  \centering
  {\scriptsize \input{tables/table_results_summary}}
...
\begin{frame}{Results — endurance effect ($n_r=3$, means)}
  \centering
  {\scriptsize \input{tables/table_results_by_endurance}}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.58\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Equivalent \textbf{Edges-Approach} DFJ reformulation
